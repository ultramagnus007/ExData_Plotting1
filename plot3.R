...
{
	#making the plot with no data in it with type = "n"
	plot(date, as.numeric(HH$Sub_metering_1),
		type = "n", xlab = "", ylab = "Energy sub metering")

	#making three plots simultaneously on using line function
	#date vs Sub_metering_1
	lines(date , as.numeric(HH$Sub_metering_1), type = "l" , col = "black")
	#date vs Sub_metering_2
	lines(date , as.numeric(HH$Sub_metering_2), type = "l", col = "red")
	#date vs Sub_metering_3
